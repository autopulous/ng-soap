...
const method: string = 'ExecuteLogin';
const requestRoot: string = `ExecuteLoginRequest`;
const responseRoot: string = `ExecuteLoginResponse`;

const servicePort: string = 'http://172.16.62.60:7002';
const serviceRoot: string = '/vertex-enterprise-ws/ws/';
const targetNamespace: string = 'http://www.vertexinc.com/gto/schema/definitions';

const username: string = 'info@vertexSMB.com';
const password: string = 'OKMzaq123*()';

let soapService: SoapService = null;

let responseJso: {} = null;
let responseXml: NodeListOf<Element> = null;
...
function dateString(date: Date): string {
  ...
  const mm: string = (10 > month ? '0' : '') + month;

  const day: number = date.getDate();
  const dd: string = (10 > day ? '0' : '') + day;
